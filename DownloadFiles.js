...
async function downloadFile(url, filePath) {
    try {
        // Configurando axios para lidar com arquivos binários
        const response = await axios({
            url,
            method: 'GET',
            responseType: 'stream' // O responseType 'stream' garante que a resposta seja tratada como um fluxo de dados binário
        });

        // Criar o arquivo e fazer o download
        const writer = fs.createWriteStream(filePath);
        response.data.pipe(writer);

        return new Promise((resolve, reject) => {
            writer.on('finish', resolve); // Resolve quando o download terminar
            writer.on('error', reject);   // Rejeita se houver um erro
        });
    } catch (error) {
